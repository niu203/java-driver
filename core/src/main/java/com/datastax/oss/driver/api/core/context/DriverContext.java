...
public interface DriverContext extends AttachmentPoint {
  ...
  Map<String, TimestampGenerator> timestampGenerators();

  default TimestampGenerator timestampGenerator(String profileName) {
    TimestampGenerator policy = timestampGenerators().get(profileName);
    return (policy != null) ? policy : timestampGenerators().get(DriverConfigProfile.DEFAULT_NAME);
  }
